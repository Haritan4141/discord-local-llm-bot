...
const ENV_SECTIONS = [
  ...
  },
  {
    ...
    fields: [
      { key: 'LLM_PROVIDER', label: 'Provider', type: 'select', options: ['ollama', 'lmstudio', 'openai', 'custom'], required: true, placeholder: 'ollama', help: 'OpenAI または公式 API URL は Responses API と公式 Web Search、それ以外は OpenAI 互換 Chat Completions を使います。' },
      ...
      { key: 'OLLAMA_WEB_API_KEY', label: 'Ollama Web Search API Key', type: 'password', placeholder: 'OpenAI 公式 API 以外の検索用です。' },
